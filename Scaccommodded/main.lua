...
-- Load title image safely
    ...
    board[4][8] = {type = pieces.mKing, color = "black"}
    startingboard[4][8] = {type = pieces.mKing, color = 'black'}
    startingslotsunlocked = 1
    KingPos = {4, 8}
    for i=1,8 do
        board[i][1] = {type = pieces.Pawn, color= "white"}
        startingboard[i][1] = {type = pieces.Pawn, color = "white"}
    end
    -- Give white a few pawns to move (so it's white's turn first)
    for i=1,8 do
        board[i][2] = {type = pieces.Pawn, color= "white"}
        startingboard[i][2] = {type = pieces.Pawn, color = "white"}
    end
    for i=1,4 do
        board[i+2][3] = {type = pieces.Pawn, color= "white"}
        startingboard[i+2][3] = {type = pieces.Pawn, color = "white"}
    end
    if args[2] then
...
local function yoinkPreviewPiece(pieceA, pieceB)
    ...
    local result = Blueprint.findResult(pieceA.piece, pieceB.piece, true)
    if result then
        return result
    end
    return nil
...
local function combinePieces()
    ...
    if preview and combinedAlready > 0 then
        ...
        if preview == pieces.Knight and InTutorial and TutorialState == 1 then
            TutorialState = 2
        elseif InTutorial and TutorialState == 10 and preview == pieces.Bishop then
            InTutorial = false
...
function startround()
    ...
    if Currentround % 5 == 0 or Currentround == 1 then
        if nextroundBoss then
            ...
            currentBossDesc = nextroundBossDesc
        ...
        for y=1,8 do
            if currentBossName == "Wall" then
                if x ~= wallrand and y == 5 then
                    board[x][y] = {type = pieces.Barrier, color = "barrier"}
                else
                    board[x][y] = nil
                end
            else
    ...
    for y=1,4 do
        for x=1,8 do
            ...
            if startpiece and startpiece.type ~= pieces.Pawn and currentBossName == "Restart" and Currentround % 5 == 0 then
                board[x][y] = {type = pieces.Pawn, color = startpiece.color}
    ...
    board[4][8] = {type = _G.currentKingType or pieces.mKing, color = "black"}
    unusedCombines = 0
    currentTurn = "white"
    local kingcheck = findBlackKing()
        ...
        if piece then
            ...
            if recipe and Piecetokens >= recipe.place then
                ...
                if pieceType then
                    ...
                    if InTutorial and TutorialState == 8 and pieceType == pieces.Knight then
                        TutorialState = 9
                    elseif InTutorial and not editingDragging then
                        InTutorial = false
                        settings.tutorial = false
                        saveSettings()
                    end
